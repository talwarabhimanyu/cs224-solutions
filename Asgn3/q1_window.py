...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        sentence_copy = [start] + sentence + [end]
        for i in range(len(sentence)):
            windowed_data.append((sum(sentence_copy[(i+1-window_size):(i+1+window_size+1):1], []), labels[i]))
	### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        self.W = tf.get_variable(name='W', shape=(self.config.n_window_features* \
