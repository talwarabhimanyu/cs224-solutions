...
def softmax(x):
    ...
    ### YOUR CODE HERE
    scores = tf.exp(tf.cast(x - tf.expand_dims(tf.reduce_max(x, axis=1), axis=1), tf.float32))
    max_score = tf.expand_dims(tf.reduce_sum(scores, axis=1), axis=1)
    out = scores/max_score
    ### END YOUR CODE
